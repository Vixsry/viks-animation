...
0;
},getDuration(element) {return element.getAttribute('data-viks-duration') || 
this.getAttributeFromString(element.getAttribute('data-viks'), 'duration-') || 
1000;
},getEasing(element) {return element.getAttribute('data-viks-easing') || this.getAttributeFromString(element.getAttribute('data-viks'), 'easing-') || 'ease';
},
...
link.addEventListener("click", function(event) {event.preventDefault();const targetId = this.getAttribute("href").substring(1);const targetSection = document.getElementById(targetId);targetSection.scrollIntoView({
behavior: "smooth" }); }); }); VIKS.init();